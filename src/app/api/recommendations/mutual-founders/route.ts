...
export async function GET(request: NextRequest) {
  try {
    ...
    const recommendations = await engine.getMutualFounders(founderId, limit);

    return NextResponse.json({
      success: true,
      graphTraversal: '2-Hop Friend-of-Friend Traversal: (FounderA)-[:CONNECTED_TO]-(Mutual)-[:CONNECTED_TO]-(FounderB)',
